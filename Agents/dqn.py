...
class DQN_Agent(BaseAgent):
    def __init__(self, n_actions:int,model:nn.Module,discount_factor=0.99, learning_rate=0.1,epsilon=0.1):
    ...
    def act(self, key: jax.random.PRNGKey,state:CTP_environment.EnvState,observation:CTP_environment.Observation,online_net_params:dict) -> int:
        ...
        action = jax.lax.cond(explore,lambda subkey:jax.random.randint(subkey, shape=(1,), minval=0, maxval=self.n_actions).astype(jnp.int32),lambda _:_forward_pass(),subkey)
        return action
    ...
    def _loss_fn(self,online_net_params,target_net_params,states,actions,rewards,next_states,terminate):
        pass
