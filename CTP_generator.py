...
def generate_graph(n_nodes: int, key: int, use_edge_weights=True, grid_size=10,prop_stoch=0.4
    ...
    edge_weights = jax.lax.cond(use_edge_weights, lambda _:jnp.linalg.norm(grid_nodes_jax[senders] - grid_nodes_jax[receivers], axis=1), lambda _:jnp.ones_like(senders,dtype=jnp.float32),operand=None)
...
def get_agent_graph(true_graph: jraph.GraphsTuple) -> jraph.GraphsTuple:
    ...
    agent_graph = true_graph._replace(edges={**true_graph.edges,'blocked_status':stochastic_edges},globals=global_context)
    return agent_graph
