...
class Beginning_CNN_Block_30_With_Concatenation(nn.Module):
    ...
    def __call__(self, x):
        ...
        x_out = nn.max_pool(x_out, window_shape=(2, 2), strides=(2, 2), padding="VALID")
        x_out.reshape(-1)
        return x_out


class Combined_Block_CNN_30_With_Concatenation(nn.Module):
    # for both the actor and critic
    @nn.compact
    def __call__(self, x: jnp.ndarray):
        out = jnp.transpose(x, (1, 2, 0))
        out = Beginning_CNN_Block_30_With_Concatenation(128)(out)  # 3 conv layers
...
class Small_Concatenate_CNN_30(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> tuple[distrax.Categorical, float]:
        ...
        critic = Combined_Block_CNN_30_With_Concatenation()(x)
        critic = nn.Dense(
            1,
