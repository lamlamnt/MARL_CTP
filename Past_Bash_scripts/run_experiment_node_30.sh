                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            stoch="${prop_stoch[$i]}"
                            log_dir="node30_fixed/seed_${seed}_prop_${stoch}_lr_${lr}_update_${num_update_epochs_value}_steps_${num_steps_before_update_value}_minibatch_${num_minibatches_value}_ent_${ent_coeff_value}_clip_${clip_eps_value}_horizon_${horizon_length_factor}"
                            python main_ppo.py --n_node 30 --time_steps 10000000 --log_directory "$log_dir" --random_seed_for_training "$seed" --prop_stoch "$stoch" --wandb_mode online --wandb_project_name node30_fixed --num_update_epochs "$num_update_epochs_value" --num_steps_before_update "$num_steps_before_update_value" --num_minibatches "$num_minibatches_value" --ent_coeff "$ent_coeff_value" --clip_eps "$clip_eps_value" --vf_coeff "$vf_coeff" --deterministic_inference_policy "$deter" --learning_rate "$lr" --horizon_length_factor "$horizon_length_factor"
                        done
                    done
                done
            done
